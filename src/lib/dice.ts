...
export function avg(expr: DiceExpr): number {
  ...
  for (const term of expr) {
    ...
    if (count <= 0 || !Number.isInteger(count))
      throw new Error("There must be at least one whole number die rolled.");
    sidesValidate(sides);
    if (plus !== undefined && (plus < 0 || !Number.isInteger(plus)))
      throw new Error("Flat modifiers must be integers.");
