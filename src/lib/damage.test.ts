...
describe("avgDamage() - damage averaging and crit tests", () => {
  ...
  it("avgDamage: Handles multiple dice components correctly", () => {
    expect(avgDamage([...componentOne, componentTwo], false)).toBe(
      7 + 13.5 + 3
    );
  ...
  it("avgDamage: Skips dice doubling when critsDoubleDice is false", () => {
    expect(
      avgDamage(
        [{ ...componentOne[0], critDoublesDice: false }, componentTwo],
        true
      )
    ).toBe(7 + 27 + 3);
  });
  ...
  it("avgDamage: Throws error on non-integer bonus", () => {
    expect(() =>
      avgDamage([{ ...componentOne[0], bonus: 2.5 }], false)
    ).toThrow("Damage bonus must be an integer.");
  });
});
