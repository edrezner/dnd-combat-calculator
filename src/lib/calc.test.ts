...
describe("hitChance extras", () => {
  ...
  it("hitChance: Hit chance is always between 0 and 1", () => {
    for (let i = 0; i < 200; i++) {
      ...
      const p = hitChance({
        attackBonus: attackBonusRandom,
        targetAC: targetACRandom,
      });
...
describe("expectedDamage basics", () => {
  ...
  it("expectedDamage: Expected damage decreases with disadvantage", () => {
    const dmgDisadv = expectedDamage({
      hitChance: 0.4225,
      critChance: 0.05,
      avgOnHit: 10,
      avgOnCrit: 20,
...
describe("expectedDamage extras", () => {
  it("expectedDamage: Advantage + disadvantage deliver same expected damage as normal", () => {
    ...
    const dmgAdvDisadv = expectedDamage({
      hitChance: pAdvDisadv,
      critChance: 0.05,
      avgOnHit: 10,
      avgOnCrit: 20,
    });
...
describe("expectedDamageFromProfile basics", () => {
  it("expectedDamageFromProfiles: Calculates damage at normal/advantage/disadvantage statuses correctly", () => {
    ...
    const disadv = expectedDamageFromProfile({
      ...profileOne,
      disadvantage: true,
    });
  ...
  it("expectedDamageFromProfile: Calculates multiple Damage Components (mixed dice) correctly", () => {
    const twoDSixPlusFourDEightPlusThree =
      expectedDamageFromProfile(profileTwo);
  ...
  it("expectedDamageFromProfile: Ensures larger crit chance increases damage and hit chance can never be 100% or 0%", () => {
    const normal = expectedDamageFromProfile(profileOne);
    const critOnNineteen = expectedDamageFromProfile({
      ...profileOne,
      critRange: 19,
    });
    const onlyCritHits = expectedDamageFromProfile({
      ...profileOne,
      attackBonus: 0,
      targetAC: 25,
    });
    const onlyOneMisses = expectedDamageFromProfile({
      ...profileOne,
      attackBonus: 30,
      targetAC: 10,
    });
  ...
  it("expectedDamageFromProfiles Empty damage component, dice term with invalid sides, and non-integer bonuses throw errors", () => {
    expect(() =>
      expectedDamageFromProfile({ ...profileOne, damage: [{ expr: [] }] })
    ).toThrow("Damage component #0 must have dice (expr) or a flat bonus.");
    expect(() =>
      expectedDamageFromProfile({
        ...profileOne,
        damage: [
          { expr: [{ count: 2, sides: 6 }], bonus: 2.5, critDoublesDice: true },
        ],
      })
    ).toThrow(`Damage component #${0} has invalid bonus: ${2.5}`);
  });
